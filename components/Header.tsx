...
interface HeaderProps {
    ...
    isReturningToBatch?: boolean;
}

const Header: React.FC<HeaderProps> = ({ onBack, showBackButton = false, isReturningToBatch = false }) => {
  return (
    <header className="w-full py-4 px-8 border-b border-gray-700 bg-gray-800/30 backdrop-blur-sm sticky top-0 z-50">
      <div className="relative flex items-center justify-center max-w-7xl mx-auto">
          {showBackButton && (
              <button
                  onClick={onBack}
                  className="absolute left-0 flex items-center gap-1.5 text-gray-300 hover:text-white transition-colors duration-200 group"
                  aria-label={isReturningToBatch ? "Go back to batch view" : "Go back to upload screen"}
              >
                  <ChevronLeftIcon className="w-5 h-5 transition-transform group-hover:-translate-x-1" />
                  <span className="font-medium">{isReturningToBatch ? 'Back to Batch' : 'Back'}</span>
              </button>
          )}
